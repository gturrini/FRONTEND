...
function loadTable() {
  ...
  xhttp.open("GET", "http://13.40.154.100/api/read_all.php");
  xhttp.send();
...
function userCreate() {
  ...
  const xhttp = new XMLHttpRequest();
  xhttp.open("POST", "http://13.40.154.100/api/create.php");
...
function showUserEditBox(id) {
  console.log(id);
  const xhttp = new XMLHttpRequest();
  xhttp.open("GET", "http://13.40.154.100/api/read_single.php/?id="+id);
  xhttp.send();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      const objects = JSON.parse(this.responseText);
...
function userEdit() {
  ...
  const xhttp = new XMLHttpRequest();
  xhttp.open("PUT", "http://13.40.154.100/api/update.php");
...
function userDelete(id) {
  const xhttp = new XMLHttpRequest();
  xhttp.open("DELETE", "http://13.40.154.100/api/delete.php");
  xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
  xhttp.send(JSON.stringify({
    "id": id
  }));
